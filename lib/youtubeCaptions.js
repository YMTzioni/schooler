...
import { fetchPubProxy, toAxiosProxy } from './pubProxy.js'
...
const MAX_PUB_PROXY_ATTEMPTS = Number(process.env.PUBPROXY_MAX_ATTEMPTS || 6)
...
export class YouTubeBlockedError extends Error {
  constructor() {
    super(
      'לא הצלחנו להוריד כתוביות מ-YouTube (גם דרך פרוקסי ציבורי). נסו שוב בעוד דקה, או הריצו מקומית עם npm start.',
    )
...
const isCloudHost = Boolean(process.env.RENDER)
...
const buildProxyAttempts = async ({ preferPubProxy = false } = {}) => {
  const attempts = []

  if (staticAxiosProxy) {
    attempts.push(staticAxiosProxy)
  }

  if (shouldUsePubProxy() && (preferPubProxy || isCloudHost)) {
    for (let index = 0; index < MAX_PUB_PROXY_ATTEMPTS; index += 1) {
      const proxy = await fetchPubProxy()
      if (proxy) attempts.push(proxy)
    }
  }

  if (!isCloudHost || !shouldUsePubProxy()) {
    attempts.push(null)
  }
...
const requestYouTube = async (configFactory, { preferPubProxy = false } = {}) => {
  const attempts = await buildProxyAttempts({ preferPubProxy })
  let lastError = null
  ...
  for (const proxy of attempts) {
    try {
      ...
      if (isRetryableProxyResponse(response)) continue
      return response
    } catch (error) {
      lastError = error
      if (!proxy || !isRetryableProxyError(error)) {
...
export const createYouTubeFetch = ({ preferPubProxy = false } = {}) => {
...
const fetchCaptionTracksInnertube = async (videoId, { preferPubProxy = false } = {}) => {
...
const fetchCaptionTracksFromWatchPage = async (videoId, { preferPubProxy = false } = {}) => {
  try {
...
export const getCaptionTrackInfo = async (videoId, { preferPubProxy = false } = {}) => {
  const innertubeInfo = await fetchCaptionTracksInnertube(videoId, { preferPubProxy })
  if (innertubeInfo?.tracks?.length) return innertubeInfo

  const watchInfo = await fetchCaptionTracksFromWatchPage(videoId, { preferPubProxy })
...
export const fetchTrackSubtitleContent = async (
  sourceTrack,
  { tlang, fmt = 'vtt', userAgent, preferPubProxy = false } = {},
) => {
...
export const fetchTranscriptSegments = async (videoId, { lang = 'auto' } = {}) => {
  ...
  try {
    const segments = await YoutubeTranscript.fetchTranscript(videoId, {
      ...transcriptLang,
      fetch: createYouTubeFetch({ preferPubProxy }),
    })
  ...
  const info = await getCaptionTrackInfo(videoId, { preferPubProxy })
